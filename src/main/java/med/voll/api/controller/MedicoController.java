...
public class MedicoController {
	...
	@PostMapping
	@Transactional
	public ResponseEntity<DadosDetalhamentoMedico> cadastrarMedico(@Valid @RequestBody DadosCadastroMedico dados, UriComponentsBuilder uriBuilder) {
		Medico medicoSalvo = medicoRepository.save(new Medico(dados));
	...
	public ResponseEntity<Page<DadosListagemMedico>> listarMedicos(@PageableDefault(size=10, sort = {"nome"}) Pageable paginacao){
		return ResponseEntity.ok(medicoRepository.findAllByAtivoTrue(paginacao).map(DadosListagemMedico::new));
	}
	
	@PutMapping
	@Transactional
	public ResponseEntity<DadosDetalhamentoMedico> atualizarMedico(@Valid @RequestBody DadosAtualizacaoMedico dados) {
